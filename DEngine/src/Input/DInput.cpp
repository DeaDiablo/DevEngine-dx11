...
float     dMouseX = -1, dMouseY = -1;
float     width = (float)GetSystemMetrics(SM_CXSCREEN) / 0xFFFF, height = (float)GetSystemMetrics(SM_CYSCREEN) / 0xFFFF;
//keyboard
bool      Keys[256];
    ...
    case RIM_TYPEMOUSE:
      {
        ...
        {
          float dx = (float)Raw->data.mouse.lLastX * width;
          float dy = (float)Raw->data.mouse.lLastY * height;
...
float Input::GetMouseDeltaZ() const
{
  float mz = MouseZ;
  MouseZ = 0;
  return mz;
...
InputStruct Input::GetInputStruct() const
{
  InputStruct is;
  memcpy(is.mouseKey,  Button, sizeof(Button));
  is.mouseAxis.x = MouseX;
  is.mouseAxis.y = MouseY;
  is.mouseAxis.z = MouseZ;
  MouseX = 0;
  MouseY = 0;
  MouseZ = 0;
  memcpy(is.key, Keys, sizeof(Keys));
  return is;
}
